...
export function AppBar({ isAdmin = false }: AppBarProps) {
  return (
    <>
      <style>{`
        .appbar{position:sticky;top:0;z-index:200;height:56px;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;padding:0 12px;background:rgba(2,2,8,.95);backdrop-filter:blur(32px) saturate(180%);border-bottom:1px solid rgba(255,255,255,.06);gap:8px}
        .appbar-left{display:flex;align-items:center;justify-content:flex-start}
        .appbar-logo{display:flex;align-items:center;justify-content:center;gap:7px;text-decoration:none;white-space:nowrap}
        ...
        .appbar-right{display:flex;align-items:center;justify-content:flex-end;gap:4px}
