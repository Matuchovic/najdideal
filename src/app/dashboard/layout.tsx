...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-void-1000">
      <style>{`
        .dn{height:60px;border-bottom:1px solid rgba(255,255,255,.06);background:rgba(2,2,8,.88);backdrop-filter:blur(32px) saturate(180%);display:grid;grid-template-columns:auto 1fr auto;align-items:center;padding:0 16px;gap:12px;overflow:hidden;position:relative}
        .dn-logo{display:flex;align-items:center;justify-content:center;justify-self:center;}
