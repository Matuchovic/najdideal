...
export async function generateMetadata({ params }: Props) {
  ...
  } catch { return { title: 'Deal | NajdiDeal' } }
}
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes glow-btn { 0%,100%{box-shadow:0 0 20px rgba(240,180,41,.3)} 50%{box-shadow:0 0 50px rgba(240,180,41,.7)} }
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="gc" style={{ padding: 20 }}>
              <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 16 }}>
                <div style={{ width: 72, height: 72, borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(240,180,41,.2)', flexShrink: 0, background: 'rgba(240,180,41,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 34 }}>
                  {deal.image_url
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                    ...
                    {deal.is_hot && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 100, fontSize: 9, fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', background: 'rgba(0,230,118,.08)', border: '1px solid rgba(0,230,118,.2)', color: '#00E676' }}>🔥 Hot</span>}
                  </div>
              ...
              {displayPrice && (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
                  <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12, padding: 14 }}>
                    ...
                    <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: '#F0B429', letterSpacing: 1 }}>{displayPrice.toLocaleString('cs-CZ')} Kč</div>
                  </div>
                  {deal.profit_amount && (
                    <div style={{ background: 'rgba(0,230,118,.05)', border: '1px solid rgba(0,230,118,.15)', borderRadius: 12, padding: 14 }}>
                      <div style={{ fontSize: 9, color: 'rgba(0,230,118,.5)', letterSpacing: '1.5px', textTransform: 'uppercase', marginBottom: 5 }}>Profit</div>
                      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 22, color: '#00E676', letterSpacing: 1 }}>+{formatCZK(deal.profit_amount)}</div>
                    </div>
                  )}
                </div>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="gc gc-fast" style={{ padding: 18 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {deal.source_url && (
                  <a href={deal.source_url} target="_blank" rel="noopener noreferrer" style={{ background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.1)', color: '#F0EBE1', borderRadius: 10, padding: '13px', fontSize: 10, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textDecoration: 'none' }}>
                    ↗ Zobrazit na Bazosi
                  </a>
                )}
                <Link href={`/api/save-deal?deal_id=${deal.id}&saved=${isSaved}&redirect=/deals/${deal.slug}`} style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.07)', color: 'rgba(240,235,225,.6)', borderRadius: 10, padding: '11px', fontSize: 10, fontWeight: 600, letterSpacing: '1.5px', textTransform: 'uppercase', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, textDecoration: 'none' }}>
            ...
            <div className="gc gc-green" style={{ padding: 18 }}>
              ...
              <p style={{ fontSize: 11, color: 'rgba(240,235,225,.6)', lineHeight: 1.6, marginBottom: 12 }}>Buď první u podobných dealů.</p>
              <Link href="/alerts" style={{ background: 'rgba(0,230,118,.12)', border: '1px solid rgba(0,230,118,.25)', color: '#00E676', borderRadius: 8, padding: '10px', fontSize: 9, fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, textDecoration: 'none' }}>
