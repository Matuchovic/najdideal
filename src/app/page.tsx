  ...
  return (
    <div style={{background:'#020208',color:G.wht,fontFamily:'Syne,sans-serif',overflowX:'hidden'}}>
      ...
      <section className="sec-pad" style={{position:'relative',zIndex:10,padding:'120px clamp(16px,5vw,56px)',background:'linear-gradient(180deg,#020208 0%,#06060E 100%)'}}>
        ...
        <div style={{marginTop:64,position:'relative',borderRadius:24,overflow:'hidden',border:'1px solid rgba(240,180,41,.22)',background:'linear-gradient(135deg,rgba(240,180,41,.04) 0%,rgba(2,2,8,.95) 60%)',animation:'vipGlow 4s ease-in-out infinite'}}>
          <style>{`
            @keyframes vipFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
            @keyframes vipShine{0%{background-position:-200% center}100%{background-position:200% center}}
            @keyframes vipPing{0%{transform:scale(1);opacity:1}100%{transform:scale(2.2);opacity:0}}
            .vip-benefit{transition:all .3s cubic-bezier(.34,1.56,.64,1)}
            .vip-benefit:hover{transform:translateY(-4px) scale(1.02)}
            .vip-plan{transition:all .35s cubic-bezier(.34,1.56,.64,1)}
            .vip-plan:hover{transform:translateY(-6px)}
          `}</style>
          <div style={{position:'absolute',top:0,left:0,right:0,height:1,background:'linear-gradient(90deg,transparent,#F0B429,transparent)'}} />
          <div style={{position:'absolute',top:-60,right:-60,width:240,height:240,background:'radial-gradient(circle,rgba(240,180,41,.08) 0%,transparent 70%)',borderRadius:'50%',pointerEvents:'none'}} />
          <div style={{padding:'48px clamp(20px,5vw,64px) 52px'}}>
            <div style={{textAlign:'center',marginBottom:40}}>
              <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'6px 16px',borderRadius:100,background:'rgba(240,180,41,.08)',border:'1px solid rgba(240,180,41,.22)',marginBottom:16}}>
                <span style={{width:7,height:7,borderRadius:'50%',background:G.g,display:'inline-block',boxShadow:'0 0 8px #F0B429',animation:'goldPulse 2s infinite',flexShrink:0}} />
                <span style={{fontFamily:'Syne Mono,monospace',fontSize:9,fontWeight:700,letterSpacing:3,textTransform:'uppercase',color:G.g}}>👑 Exkluzivní VIP přístup</span>
              </div>
              <h3 style={{fontFamily:'Bebas Neue,sans-serif',fontSize:'clamp(36px,6vw,72px)',letterSpacing:4,color:G.wht,lineHeight:.9,marginBottom:12}}>ZÍSKEJ PŘÍSTUP DŘÍV<br/><span style={{background:'linear-gradient(90deg,#F0B429 0%,#FFD97D 40%,#F0B429 80%,#C8880A 100%)',backgroundSize:'200% auto',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',animation:'vipShine 3s linear infinite'}}>NEŽ OSTATNÍ</span></h3>
              <p style={{fontSize:13,color:G.mut,fontWeight:300,lineHeight:1.8,maxWidth:560,margin:'0 auto'}}>Průměrný VIP člen vydělá <span style={{color:G.g,fontWeight:600}}>4 235 Kč měsíčně navíc</span>. Alerty přicházejí 2 hodiny před free uživateli.</p>
            </div>

            {/* Benefits grid */}
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,marginBottom:40}}>
              {[
                {i:'⚡',t:'Instant alerty',d:'2h před ostatními'},
                {i:'🤖',t:'AI příležitosti',d:'Exkluzivní AI dealy'},
                {i:'🛒',t:'Marketplace',d:'Přidávej inzeráty'},
                {i:'👥',t:'Komunita',d:'Soukromá skupina'},
                {i:'📊',t:'Statistiky',d:'Přehled tvých dealů'},
                {i:'🔔',t:'Hlídač ceny',d:'Upozornění na slevy'},
              ].map((b,i) => (
                <div key={i} className="vip-benefit" style={{background:'rgba(240,180,41,.04)',border:'1px solid rgba(240,180,41,.12)',borderRadius:14,padding:'16px 14px',textAlign:'center'}}>
                  <div style={{fontSize:22,marginBottom:8}}>{b.i}</div>
                  <div style={{fontFamily:'Syne Mono,monospace',fontSize:8,fontWeight:700,letterSpacing:2,textTransform:'uppercase',color:G.g,marginBottom:4}}>{b.t}</div>
                  <div style={{fontSize:10,color:G.mut,fontWeight:300}}>{b.d}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:14,marginBottom:28}}>
              <div className="vip-plan" style={{background:'rgba(255,255,255,.03)',border:'1px solid rgba(240,180,41,.2)',borderRadius:18,padding:'28px 24px',position:'relative',overflow:'hidden'}}>
                ...
                <div style={{fontFamily:'Syne Mono,monospace',fontSize:9,fontWeight:700,letterSpacing:3,textTransform:'uppercase',color:G.g,marginBottom:8}}>⭐ STANDARD</div>
                <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:52,color:G.g,letterSpacing:2,lineHeight:1,marginBottom:4}}>299 <span style={{fontSize:20}}>Kč</span></div>
                <div style={{fontFamily:'Syne Mono,monospace',fontSize:9,color:G.mut,letterSpacing:1,marginBottom:16}}>za měsíc · zruš kdykoliv</div>
                <ul style={{listStyle:'none',padding:0,margin:'0 0 20px',display:'flex',flexDirection:'column',gap:6}}>
                  {['Okamžitá upozornění','Kontakt na prodejce','Až 5 inzerátů/měsíc','Soukromá komunita','Hlídač ceny'].map(f => <li key={f} style={{fontSize:11,color:'rgba(240,235,225,.7)',display:'flex',alignItems:'center',gap:7}}><span style={{color:G.grn,fontSize:10}}>✓</span>{f}</li>)}
                </ul>
                <Link href="/vip" style={{display:'block',textAlign:'center',background:'rgba(240,180,41,.12)',color:G.g,fontFamily:'Syne Mono,monospace',fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',padding:'12px',borderRadius:8,textDecoration:'none',border:'1px solid rgba(240,180,41,.25)',transition:'all .3s'}}>Vybrat Standard →</Link>
              </div>
              <div className="vip-plan" style={{background:'rgba(240,180,41,.06)',border:'2px solid rgba(240,180,41,.35)',borderRadius:18,padding:'28px 24px',position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',top:-1,left:'50%',transform:'translateX(-50%)',background:G.g,color:'#000',fontFamily:'Syne Mono,monospace',fontSize:7,fontWeight:700,letterSpacing:2,textTransform:'uppercase',padding:'4px 14px',borderRadius:'0 0 8px 8px'}}>Nejoblíbenější</div>
                <div style={{position:'absolute',top:0,left:'10%',right:'10%',height:1,background:'linear-gradient(90deg,transparent,#F0B429,transparent)'}} />
                <div style={{fontFamily:'Syne Mono,monospace',fontSize:9,fontWeight:700,letterSpacing:3,textTransform:'uppercase',color:G.g,marginBottom:8,marginTop:8}}>💎 PREMIUM</div>
                <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:52,color:G.g,letterSpacing:2,lineHeight:1,marginBottom:4,textShadow:'0 0 30px rgba(240,180,41,.4)'}}>699 <span style={{fontSize:20}}>Kč</span></div>
                <div style={{fontFamily:'Syne Mono,monospace',fontSize:9,color:G.mut,letterSpacing:1,marginBottom:16}}>za měsíc · zruš kdykoliv</div>
                <ul style={{listStyle:'none',padding:0,margin:'0 0 20px',display:'flex',flexDirection:'column',gap:6}}>
                  {['Vše ze Standard','Alerty JAKO PRVNÍ','Až 20 inzerátů/měsíc','1× Boost zdarma/měsíc','Odhad správné ceny','Prioritní podpora 4h'].map(f => <li key={f} style={{fontSize:11,color:'rgba(240,235,225,.7)',display:'flex',alignItems:'center',gap:7}}><span style={{color:G.g,fontSize:10}}>✓</span>{f}</li>)}
                </ul>
                <Link href="/vip" style={{display:'block',textAlign:'center',background:G.g,color:'#000',fontFamily:'Syne Mono,monospace',fontSize:9,fontWeight:700,letterSpacing:2,textTransform:'uppercase',padding:'12px',borderRadius:8,textDecoration:'none',boxShadow:'0 8px 28px rgba(240,180,41,.35)',transition:'all .3s'}}>🔒 Vstoupit do Premium →</Link>
              </div>
            </div>
            <div style={{display:'flex',justifyContent:'center',gap:28,flexWrap:'wrap'}}>
              {['✓ Zruš kdykoliv','✓ Bezpečná platba','✓ Okamžitý přístup','✓ Žádné závazky'].map(t => <span key={t} style={{fontFamily:'Syne Mono,monospace',fontSize:9,color:G.grn}}>{t}</span>)}
            </div>
          </div>
        </div>
      </section>
